...
```bash
yarn admin stripe init-products -k <stripe_api_secret_key>
```
...
Copy the `priceId` properties from both the `proMembership` and `shopOwnerMembership` objects and use those values to
set the `BT_STRIPE_PRO_MEMBERSHIP_PRICE` and `BT_STRIPE_SHOP_OWNER_MEMBERSHIP_PRICE` environment variables. These values
are needed to create new subscriptions.
...
```bash
yarn admin stripe webhooks -k <stripe_api_secret_key> <endpoint_url>
```
