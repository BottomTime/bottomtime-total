...
export const CertificationSchema = z.object({
  agency: z.string().trim().max(100).nullable().optional().openapi({
    title: 'Agency',
  ...
  date: z.string().trim().regex(DateRegex).nullable().optional().openapi({
    title: 'Date Received',
...
export const UpdateProfileParamsSchema = z
  .object({
    ...
    certifications: CertificationSchema.array().max(200).openapi({
...
export const ProfileSchema = UpdateProfileParamsSchema.extend({
  userId: z.string().uuid().openapi({
    title: 'User ID',
    description:
      'The globally-unique ID that uniquely identifies the user account to which this profile belongs.',
  }),
