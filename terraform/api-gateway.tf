...
### EDGE Authenticator
resource "aws_apigatewayv2_api" "edge_authenticator" {
  name          = "bottomtime-authenticator-${var.env}"
  protocol_type = "HTTP"
...
resource "aws_apigatewayv2_domain_name" "edge_authenticator" {
  domain_name = "${var.authentication_domain}.${var.root_domain}"
...
resource "aws_apigatewayv2_integration" "edge_authenticator_lambda" {
  api_id                 = aws_apigatewayv2_api.edge_authenticator.id
  integration_type       = "AWS_PROXY"
  integration_method     = "POST"
  integration_uri        = aws_lambda_function.edge_authenticator.invoke_arn
  payload_format_version = "2.0"
}

resource "aws_apigatewayv2_route" "edge_authenticator" {
  api_id    = aws_apigatewayv2_api.edge_authenticator.id
  route_key = "ANY /{proxy+}"
  target    = "integrations/${aws_apigatewayv2_integration.edge_authenticator_lambda.id}"
}

resource "aws_apigatewayv2_stage" "edge_authenticator" {
  api_id      = aws_apigatewayv2_api.edge_authenticator.id
  name        = "$default"
  auto_deploy = true

  access_log_settings {
    destination_arn = aws_cloudwatch_log_group.edge_authenticator_logs.arn
    format = jsonencode({
      requestId               = "$context.requestId"
      sourceIp                = "$context.identity.sourceIp"
      requestTime             = "$context.requestTime"
...
resource "aws_apigatewayv2_api_mapping" "service_edge_authenticator_domain" {
  api_id      = aws_apigatewayv2_api.edge_authenticator.id
  domain_name = aws_apigatewayv2_domain_name.edge_authenticator.id
  stage       = aws_apigatewayv2_stage.edge_authenticator.id
}
